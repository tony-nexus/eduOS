...
const DEFAULTS = {
  ...
  toast: {
    ...
    closeHoverColor:'rgba(255,255,255,0.8)',
    progressH:     '2px',
    progressRadius:'0 0 10px 10px',
    iconSize:      '18px',
    iconRadius:    '0',
    actionColor:   null,
    actionFontSize:'12px',
...
function dismissToast(id) {
  ...
  setTimeout(() => {
    el.style.transition = 'all 0.24s ease-out';
...
function createToast(message, options = {}) {
  ...
  if (o.sound && getSoundEnabled()) playSound(type);
  ...
  el.innerHTML = `
    ${showIcon ? `<div style="width:${iconSize};height:${iconSize};min-width:${iconSize};flex-shrink:0;color:${iconColor};display:flex;align-items:center;justify-content:center;margin-top:1px">${icon}</div>` : ''}
...
export function getSoundEnabled() {
  try { return localStorage.getItem('notif_sound_enabled') !== 'false'; }
  catch { return true; }
}
export function setSoundEnabled(v) {
  try { localStorage.setItem('notif_sound_enabled', v ? 'true' : 'false'); }
  catch {}
  DEFAULTS.sound = !!v;
}
