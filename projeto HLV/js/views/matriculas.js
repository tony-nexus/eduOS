...
function applyFilter() {
  ...
  tbody.innerHTML = f.map(m => `
    <tr>
      ...
      <td style="font-size:12px;color:var(--text-tertiary)">${fmtDate(m.data_matricula || m.created_at)}</td>
      <td>
        <div style="display:flex;flex-direction:column;gap:4px">
          <span class="badge ${BADGE_MAP[m.status] ?? 'badge-gray'}">${LABEL_MAP[m.status] ?? m.status}</span>
          ${m.data_conclusao ? `<span style="font-size:10px;color:var(--text-tertiary)">Concluído: ${fmtDate(m.data_conclusao)}</span>` : ''}
        </div>
      </td>
      <td>
